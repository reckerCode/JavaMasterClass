...
//        Worse, you're pretty sure it's not job-related -- they're both huge fans of the space soap opera
//        ""Lance & Janice"". You know how much Commander Lambda hates waste, so if you can prove that these minions
//        are wasting time passing non-job-related notes, it'll put you that much closer to a promotion.
//
...
//        'y', 'c' becomes 'x', etc.  For instance, the word ""vmxibkgrlm"", when decoded, would become ""encryption"".
//
...
//      **************Submitted solution*******************
        ...
        while(i!=x.length()){
            if((int)x.charAt(i)>=97 && (int)x.charAt(i)<=122){
                int temp2 = ((x.charAt(i)-'z')+25)%26;
                decrypted.append((char) ('z' - temp2));
            }else{
                decrypted.append(x.charAt(i));
            }i++;
        }
